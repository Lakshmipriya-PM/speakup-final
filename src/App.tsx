...
const CATEGORIES: Category[] = [
  { id: "Career", name: "Career", icon: Briefcase, color: "text-orange-400" },
  { id: "Self Improvement", name: "Self Improvement", icon: Zap, color: "text-orange-400" },
  { id: "Life & Relationships", name: "Life & Relationships", icon: Heart, color: "text-orange-400" },
  { id: "Technology", name: "Technology", icon: Cpu, color: "text-orange-400" },
  { id: "Creativity", name: "Creativity", icon: Palette, color: "text-orange-400" },
  { id: "Debate", name: "Debate", icon: MessageSquare, color: "text-orange-400" },
];

// Sub-components moved outside to prevent re-creation on every App render
const CategoryButton = ({ cat, onClick }: { cat: Category, onClick: (id: string) => void | Promise<void>, key?: string }) => (
  <motion.button
...
export default function App() {
  ...
  const startRecording = () => {
    console.log("Starting session for topic:", currentTopic);
    setView("recording");
    setIsRecording(true);
    setIsPaused(false);
    setTranscript("");
  ...
  const stopRecording = () => {
    if (!isRecording && !isPaused) return; // Prevent multiple calls
    ...
    // Transition to feedback view immediately to show loading state there
    setView("feedback");
    getFeedback();
  };
  ...
  const getFeedback = async () => {
    ...
    try {
      ...
      setFeedback({
        fillers: "Unable to analyze fillers",
        clarity: "Retry needed",
        pace: "N/A",
        structure: "Feedback generation timed out or failed. Please try again.",
        tip: "Check your internet connection or verify your Gemini API key.",
        encouragement: "Stay focused! Your progress is what matters."
      });
    } finally {
      setIsLoading(false);
    }
  ...
  // Moved all view components inside to maintain access to state for now, 
  // but we'll use conditional rendering to avoid re-creating function definitions in the render loop.
    ...
    <div className="min-h-screen bg-dark-bg text-white font-sans relative overflow-x-hidden">
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <div className="max-w-6xl mx-auto px-6 sm:px-12 py-12 relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {CATEGORIES.map((cat) => (
                  <CategoryButton key={cat.id} cat={cat} onClick={fetchTopic} />
                ))}
            ...
            <div className="min-h-screen flex flex-col items-center justify-center p-12 relative z-10 bg-black">
              <div className="text-center">
                <motion.div 
                  ...
                  className="relative w-48 h-48 sm:w-64 sm:h-64 mb-12 flex items-center justify-center mx-auto"
                >
                  <svg className="w-full h-full absolute inset-0 -rotate-90">
                    ...
                    <motion.circle
                      ...
                      animate={{ strokeDashoffset: `${283 - (283 * (30 - prepTime)) / 30}%` }}
                      transition={{ duration: 1, ease: "linear" }}
                    />
                  </svg>
                  <div className="flex flex-col items-center">
                    <span className="text-4xl sm:text-8xl font-black tracking-tighter text-white">{prepTime}</span>
                    <span className="text-[8px] sm:text-[10px] uppercase font-bold tracking-[0.2em] sm:tracking-[0.4em] text-gray-500">Seconds Prep</span>
                  </div>
                </motion.div>
        ...
        {view === "recording" && (
          ...
          >
            <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-12 relative z-10">
              <motion.div 
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="w-full max-w-4xl bg-card-bg border border-white/5 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col"
              >
                <div className="p-6 sm:p-16 text-center">
                  <div className="flex justify-center mb-6">
                    <div className={`px-4 py-1.5 rounded-full flex items-center gap-2 border ${isPaused ? "bg-yellow-500/10 border-yellow-500/20 text-yellow-500" : "bg-orange-500/10 border-orange-500/20 text-orange-500"}`}>
                      <div className={`w-2 h-2 rounded-full ${isPaused ? "bg-yellow-500" : "bg-orange-500 animate-pulse"}`}></div>
                  ...
                  <h2 className="text-xl sm:text-3xl lg:text-4xl font-black text-white leading-tight mb-8 sm:mb-12 max-w-2xl mx-auto tracking-tight">
                    "{currentTopic}"
                  </h2>
                  
                  <div className="flex flex-col items-center gap-8">
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-5xl sm:text-7xl font-black font-mono tracking-tighter text-white">
                        {String(Math.floor(duration / 60)).padStart(2, "0")}:{String(duration % 60).padStart(2, "0")}
                      </span>
                      <span className="text-[9px] sm:text-[10px] uppercase font-bold tracking-[0.3em] text-gray-500">Duration</span>
                    </div>
                    ...
                    <div className="flex items-center gap-6 mt-4">
                      {isPaused ? (
                        <motion.button
                          ...
                          className="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center bg-white/10 border border-white/20 text-white"
                        >
                        ...
                        <motion.button
                          ...
                          className="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center bg-white/5 border border-white/10 text-white"
                        >
                      ...
                      <div className="relative">
                        ...
                        <motion.button
                          ...
                          className="relative z-10 w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center transition-all bg-orange-500 shadow-[0_0_40px_rgba(249,115,22,0.4)]"
                        >
                ...
                <div className="px-6 sm:px-16 pb-8 sm:pb-16 flex-1 flex flex-col">
                  <div className="bg-black/40 border border-white/5 rounded-2xl p-6 sm:p-8 min-h-[160px] sm:min-h-[200px] relative flex flex-col">
                    ...
                    <div className="mt-8 flex-1 flex items-center justify-center">
                      <p className="text-gray-400 leading-relaxed font-medium italic text-base sm:text-lg text-center max-w-2xl">
                        {transcript || (isPaused ? "Recording paused..." : "Keep speaking, I'm transcribing everything...")}
                      </p>
                    </div>
                  </div>
                  <div className="mt-6 text-center">
                    <p className={`text-[9px] sm:text-[10px] uppercase font-bold tracking-widest italic transition-colors ${isPaused ? "text-gray-700" : "text-gray-500 animate-pulse"}`}>
                      {isPaused ? "AI Coach is waiting..." : "AI Coach is listening..."}
                    </p>
                  </div>
                </div>
              </motion.div>
          ...
          <motion.div key="feedback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="max-w-4xl mx-auto px-6 sm:px-12 py-12 relative z-10">
              <div className="flex items-center justify-between mb-8 sm:mb-12">
                ...
                >
                  <ChevronLeft size={14} /> New Session
                </button>
              ...
              {!isLoading && (
                <div className="mt-12 sm:mt-16 flex flex-col sm:flex-row justify-center items-center gap-4">
                  {feedback?.clarity === "Retry needed" && (
                    <button 
                      onClick={getFeedback}
                      className="px-8 sm:px-10 py-4 sm:py-5 bg-yellow-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] sm:text-xs shadow-xl shadow-yellow-600/20 hover:scale-105 transition-all flex items-center gap-2"
                    >
